...
func (o *RenewalCredentialURL) Build() (*url.URL, error) {
	...
	var _path = "/credential/{credentialId}"

	credentialID := o.CredentialID
	if credentialID != "" {
		_path = strings.Replace(_path, "{credentialId}", credentialID, -1)
	} else {
		return nil, errors.New("CredentialID is required on RenewalCredentialURL")
	}
...
func (o *RenewalCredentialURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on RenewalCredentialURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on RenewalCredentialURL")
	}
